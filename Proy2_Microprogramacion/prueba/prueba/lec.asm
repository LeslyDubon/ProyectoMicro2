...
.data
;definicion de datos
;---Direcci�n de archivo de lectura
FileName byte "test2.txt",0
;---Cadenas de impresi�n
...
start:
	...
	main proc
		;---Ingreso de la palabra de b�squeda
		invoke StdOut, addr ingreso
		invoke StdIn,addr buffer,100

		;---Creaci�n del handler para leer el archivo
		...
		;---Determinar el tama�o del archivo
	...
	Buscar proc
		...
		diferentes:
		;---Se obtiene el siquiente caracter del archivo y se realiza de nuevo la comparaci�n
		INC edi
		MOVZX eax, BYTE PTR [edi]
		CMP eax, 0
		JE evaluar
		...
		INC esi
		INC edi
		;---Se obtiene el siquiente caracter del archivo y de la palabra. Se realiza de nuevo la comparaci�n
		MOVZX ebx, BYTE PTR [esi]
		MOV temp, ebx
		MOV ebx, temp
		CMP ebx, 0
		...
		evaluar:
		;---Evaluaci�n de al menos 1 resultado
		CMP bEncontrado,0
		JE noencontrada
		JMP Fin

		noencontrada:
		;---Impresi�n de la cadena no encontrada
		invoke StdOut, addr noencontrado
		JMP Fin

		encontrada:
		;---Impresi�n de la cadena encontrada junto a la fecha y vuelve a comenzar
	...
	;---Funciones de conversi�n a may�sculas
	EAXmayuscula proc
	CMP AX,61H
	JB mayuscula
	CMP AX,7AH
